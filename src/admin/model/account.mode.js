...
const {ROLE, TABLE} = require('../constants');
...
const getUserInfo = (result) => {
    ...
    result.third_part = _.isString(result.third_part) ? JSON.parse(result.third_part) : result.third_part;
    return result;
}
...
const prismaModel = {
    ...
    async updateUserById(id, params) {
        await prisma.user.update({
            where: {id}, data: {
                ...params, third_part: JSON.stringify(params.third_part)
            }
        })
    },
    async createUser(data) {
