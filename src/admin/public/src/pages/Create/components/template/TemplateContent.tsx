...
import { Form, Radio, Field, Input, Divider, Switch, Icon, Button } from '@alicloud/console-components';
import { FORM_ITEM_LAYOUT } from '@/constants';
import AuthDialog from '../github/AuthDialog';
import Repo from '../github/Repo';
import Trigger from '../github/Trigger';
...
const TemplateContent = (props: IProps) => {
  const { field, template, forceUpdate = noop } = props;
  ...
  return (
    <>
      <h1><Icon type="wind-arrow-left" className='mr-16 cursor-pointer' onClick={goTemplateList} />模版列表</h1>
      <Form field={field} {...FORM_ITEM_LAYOUT}>
        <FormItem label="应用模版" required>
          <span className='text-bold'>{template}</span>
          <div>
            <Button text type="primary" className='mr-8'>查看详情</Button>
            <Button text type="primary" onClick={goTemplateList}>重新选择模版</Button>
          </div>
        </FormItem>
