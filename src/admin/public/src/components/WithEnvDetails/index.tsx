...
const withEnvDetails = (WrappedComponent, { pageType }) => {
  return ({
    match: {
    ...
    return (
          ...
          <>
            <CreateEnv
              ...
              callback={async (value) => {
                history?.push(`/${orgName}/application/${appId}/${value}/${pageType}`);
                forceUpdate(Date.now());
              }}
            >
      ...
      >
        <Loading visible={loading} inline={false} className="mt-16">
          <WrappedComponent {...{ detailInfo, handleRefresh, appId, envName, orgName }} />
        </Loading>
      </PageLayout>
    );
  };
